...
async def main(args):
        ...
        if len(input_batches) > batch_size:
            for start_index in tqdm(range(0, len(input_batches), batch_size)):
                ...
                for _ in range(sample_limit):  # Generate sample_limit times for each input
                    ...
                    output_list_1.extend(batch_output)
        else:
    ...
    t_start = time.time()
    output_list_1 = await first_stage_generate_outputs(llm, input_list, model_path, max_tokens, sample_limit, temperature, top_p, batch_size)
