...
def main():
    ...
    by_sample = defaultdict(lambda: {
        "True": 0,   # total steps_above_threshold where math_equal==True
        "False": 0,  # total steps_above_threshold where math_equal==False
    ...
    for fp in files:
        ...
        steps_above_threshold = meta.get("steps_above_threshold")

        if sid is None or math_equal is None or steps_above_threshold is None:
            continue

        me_key = "True" if bool(math_equal) else "False"
        try:
            n = int(steps_above_threshold)
        except Exception:
            continue
    ...
    summary = {
        ...
        "by_sample_avg_steps_above_threshold": by_sample_avg,
        "totals": {
            "steps_above_threshold": {"True": global_total["True"], "False": global_total["False"]},
            "counts_by_math_equal": {"True": global_counts["True"], "False": global_counts["False"]},
        },
        "overall_avg_steps_above_threshold_per_file": overall_avg_steps_per_file,
        "details_per_file": per_file,  # optional
    }

    out_path = os.path.join(
        BASE_DIR, f"steps_above_threshold_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
