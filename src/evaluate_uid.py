...
from utils.calculate_uid import uid_variance, uid_gini, uid_shannon
...
def evaluate_predictions(output, labeled_answer, model_path, mode='gen'):
    final_metric = {"is_valid_answer": False, "acc": 0, "em": 0, "f1": 0, 'math_equal': 0, 'uid_variance': 0, 'uid_gini': 0, 'uid_shannon': 0}
    ...
    if mode == 'qa':
        ...
        uid_variance_score = uid_variance(output, model_path)
        uid_gini_score = uid_gini(output, model_path)
        uid_shannon_score = uid_shannon(output, model_path)
        final_metric['uid_variance'] = uid_variance_score
        final_metric['uid_gini'] = uid_gini_score
        final_metric['uid_shannon'] = uid_shannon_score
    ...
    if dataset_name == 'livecode':
        ...
        for question_idx, (item, input_prompt) in enumerate(zip(filtered_data, input_list)):
            ...
            for idx in range(len(question_samples)):
                ...
                metric, pred_answer = evaluate_predictions(output=item[f'Output_{idx}'], labeled_answer=labeled_answer, model_path=model_path, mode=mode)
