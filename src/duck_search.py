...
def duck_web_search(query, endpoint, market='en-US', language='en', timeout=20):
    """
    Perform a search using the Duck Web Search API with a set timeout.

    Args:
        query (str): Search query.
        subscription_key (str): Subscription key for the Duck Search API.
        endpoint (str): Endpoint for the Duck Search API.
    ...
    try:
        ...
        print(f"Duck Web Search request timed out ({timeout} seconds) for query: {query}")
        return {}  # Or you can choose to raise an exception

    except requests.exceptions.RequestException as e:
        print(f"Error occurred during Duck Web Search request: {e}")
        return {}

#Choice 2,web search agent
def duck_web_search_with_agent(query: str, model_name: str = "gpt-4o-mini"):
...
def extract_relevant_info(query: str, result: str):
    """
    ...
    if 'webPages' in result and 'value' in result['webPages']:
        for id, result in enumerate(result['webPages']['value']):
            info = {
                'id': id + 1,
...
def fetch_page_content(url: str, max_workers: int = 4, use_jina: bool = False, snippets: Optional[dict] = None):
    """
...
def extract_snippet_with_context(full_text: str, snippet: str, context_chars: int = 2500) -> Tuple[bool, str]:
    """
    Extract the snippet with context from the search result.
    """
    try:
        full_text = full_text[:50000]
...
if __name__ == "__main__":
    query = "Fetch me the lastest news about the stock market"

    duck_endpoint = "https://api.duckduckgo.com/"

    print("Performing DuckDuckGo Web Search...")
    result = duck_web_search(query, duck_endpoint, market='en-US', language='en')
    print(result)

    print("Extracting relevant information from search results...")
    extracted_info = extract_relevant_info(query, result)
