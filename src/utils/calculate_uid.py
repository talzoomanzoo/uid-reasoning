...
def visualize_id_vectors(uid_equal, uid_lp, uid_h, uid_d, dataset_name, model_path, split, title="ID Scores Across Steps", save_path=None):
    ...
    if save_path is None:
        ...
        save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"id_scores_across_steps_{dataset_name}.{split}.{model_path}_{timestamp}.png")
...
def visualize_average_step_counts(data, dataset_name, model_path, split, thinkseg=False):
    ...
    # Save overall plot
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    overall_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_step_count_{dataset_name}.{split}.{model_path}_{timestamp}.png")
    ...
    if math_equal_step_counts['True'] or math_equal_step_counts['False']:
        ...
        math_equal_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_step_count_by_correctness_{dataset_name}.{split}.{model_path}_{timestamp}.png")
    ...
    if dataset_name == 'math500' and level_step_counts:
        ...
        # Save level plot
        level_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_step_count_by_level_{dataset_name}.{split}.{model_path}_{timestamp}.png")
        ...
        # Save domain plot
        domain_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_step_count_by_domain_{dataset_name}.{split}.{model_path}_{timestamp}.png")
...
def visualize_average_id_vectors(data, dataset_name, model_path, split, thinkseg=False, step_limit=200):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_id_scores_{dataset_name}.{split}.{model_path}_{timestamp}.png")
    ...
    for correctness, vectors in math_equal_vectors.items():
        ...
        correctness_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_id_scores_{dataset_name}.{split}.{correctness_label.lower()}.{model_path}_{timestamp}.png")
    ...
    if dataset_name == 'gpqa' and domain_vectors:
        for domain, vectors in domain_vectors.items():
            ...
            domain_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_id_scores_{dataset_name}.{split}.domain_{domain}.{model_path}_{timestamp}.png")
    ...
    # For math500, also visualize by level
        ...
        for level, vectors in level_vectors.items():
            ...
            level_save_path = os.path.join(project_root, "scripts", "outputs", "runs.baselines", f"avg_id_scores_{dataset_name}.{split}.level_{level}.{model_path}_{timestamp}.png")
