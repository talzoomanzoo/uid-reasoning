...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=['gpqa', 'math500', 'aime', 'amc', 'livecode', 'nq', 'triviaqa', 'hotpotqa', '2wiki', 'musique', 'bamboogle', 'medbullets', 'pubhealth', 'medqa', 'jama_full', 'medxpertqa'],
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="Whether to use DuckSearch for document fetching."
    )
...
async def async_run_generation(sequences: List[Dict], max_tokens: int, batch_size: int, data_limit: int, temperature: float, top_p: float, top_k_sampling: int, repetition_penalty: float, tokenizer: AutoTokenizer, llm: LLM, sampling_params):
    # ---------------------- Generation Function ----------------------
    ...
    if len(prompts) > batch_size:
        for start_index in tqdm(range(0, len(prompts), batch_size)):
            cur_prompts = prompts[start_index:start_index + batch_size]
            batch_output = await (asyncio.to_thread(llm.generate, cur_prompts, sampling_params=sampling_params))
            output_list.extend(batch_output)
    else:
        output_list = await (asyncio.to_thread(llm.generate, prompts, sampling_params=sampling_params))
...
async def generate_webpage_to_reasonchain_batch(
        ...
        output = await (asyncio.to_thread(llm.generate, prompts, sampling_params=sampling_params))

        raw_outputs = [out.outputs[0].text for out in output] #should be checked
    ...
    # Main loop until all sequences are finished or maximum turns reached
        while True:
            ...
            if sequences_needing_generation:
                ...
                outputs = await async_run_generation(sequences_needing_generation, max_tokens, batch_size, data_limit, temperature, top_p, top_k_sampling, repetition_penalty, tokenizer, llm, sampling_params)
            ...
            # Process each sequence and collect URLs
                for seq, out in zip(sequences_needing_generation, outputs):
                    #print("output text check") #outputs[0].outputs[0].text
...
async def main_async():
    ...
    if dataset_name in ['nq', 'triviaqa', 'hotpotqa', 'musique', 'bamboogle', '2wiki', 'medmcqa', 'pubhealth',  'medbullets','medqa', 'jama_full', 'medxpertqa']:
        MAX_SEARCH_LIMIT = 5
    ...
    elif dataset_name in ['medbullets', 'pubhealth', 'medqa', 'jama_full', 'medxpertqa']:
    ...
    for item in filtered_data:
        ...
        if dataset_name in ['nq', 'triviaqa', 'hotpotqa', 'musique', 'bamboogle', '2wiki']:
            ...
            instruction = get_medical_search_o1_instruction(MAX_SEARCH_LIMIT)
            if 'qwq' in model_path.lower():
                user_prompt = get_task_instruction_medical(question, options, model_name='qwq')
            else:
                user_prompt = get_task_instruction_medical(question, options)
        else:
