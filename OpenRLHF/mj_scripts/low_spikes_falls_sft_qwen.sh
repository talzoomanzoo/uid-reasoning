...
deepspeed --num_gpus 4 --module openrlhf.cli.train_sft \
   --max_len=32768 \
   --dataset=talzoomanzoo/QWEN3_LOWEST_SPIKES_FALLS \
   --input_key=Question \
   --output_key=Output_lowest \
   --train_batch_size=32 \
   --micro_train_batch_size=1 \
   --pretrain=Qwen/Qwen3-0.6B \
   --save_path=./checkpoint/qwen3-0.6b-lowest-spikes-falls-fft \
   --packing_samples \
   --flash_attn \
   --logging_steps=1 \
   --eval_steps=-1 \
   ...
   --wandb_run_name=qwen3-0.6b-lowest-spikes-falls-fft-$(date +%m%d%H%M)