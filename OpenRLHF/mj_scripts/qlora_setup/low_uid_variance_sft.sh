...
deepspeed --num_gpus 4 --module openrlhf.cli.train_sft \
   ...
   --ring_head_stride=1 \
   --max_len=32768 \
   --dataset=talzoomanzoo/lowest_uid_variance_sft_ds-1.5b \
   --input_key=Question \
   --output_key=text \
   --train_batch_size=16 \
   --micro_train_batch_size=4 \
   --pretrain=deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B \
   --save_path=./checkpoint/deepseek-1.5b-lowest-uid-variance-ds-1.5b    \
   --packing_samples \
   --logging_steps=1 \
   --eval_steps=-1 \
   --zero_stage=2 \
   ...
   --wandb_run_name=deepseek-1.5b-lowest-uid-variance-ds-1.5b-$(date +%m%d%H%M)