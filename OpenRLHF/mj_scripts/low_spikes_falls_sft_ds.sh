...
deepspeed --num_gpus 4 --module openrlhf.cli.train_sft \
   --max_len=32768 \
   --dataset=talzoomanzoo/DS_QWEN_HIGHEST_VARIANCE \
   --input_key=Question \
   --output_key=Output_highest \
   --train_batch_size=8 \
   --micro_train_batch_size=2 \
   --pretrain=deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B \
   --save_path=./checkpoint/deepseek-1.5b-highest-uid-variance-ds-1.5b-fft \
   --packing_samples \
   --logging_steps=1 \
   --eval_steps=-1 \
   --zero_stage=2 \
   ...
   --wandb_run_name=deepseek-1.5b-highest-uid-variance-ds-1.5b-fft-$(date +%m%d%H%M)